...
const actions = {
  ...
  async [actionTypes.GET_WEATHER] ({ commit, state }, location) {
    commit(mutationTypes.SET_WEATHER, await api.getWeather(location))
  },
...
const mutations = {
  ...
  [mutationTypes.SET_WEATHER] (state, data) {
    let timezone = tzlookup(data.coord.lat, data.coord.lon)
    // console.log(timezone)
    // let currentTime = moment(data.dt * 1000).format('HH:MM:SS, dddd Do MMMM YYYY')
    // console.dir(currentTime)
    state.weather = data
    state.weather.formated = moment.tz(data.dt * 1000, timezone).format('dddd Do MMMM YYYY')
